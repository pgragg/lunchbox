...
# Should count lunch totals on a date for students or fac for any given grade range, or nil grade. 
  def self.count_by_date_and_grade(num, date, menu_id, grade_range, role, decision=nil)
    ...
    if role == "students"
      ...
      grade_ids = User.ids_in_grade(grade_range) 
      faculty_ids = User.ids_in_role(role)
      #Faculty ids will be used only if they belong to a certain grade_range. 
      #nil for non-sp. deliv. [grade_range] for sp. deliv. [[grade_range],nil].flatten for both
